...
class BertGCNForMultiLabel(nn.Module):
    """
    BERT + GCN for multi-label classification.
    Combines BERT text encoding with GCN-based label graph modeling.
    Input: token_ids (batch, seq_len), attention_mask (batch, seq_len), adj (num_labels, num_labels)
    Output: logits (batch, num_labels)
    """

    def __init__(self, bert_model='bert-base-chinese', num_labels=100, label_dim=256, dropout=0.1):
        super().__init__()
    ...
    def forward(self, input_ids, attention_mask, adj):
        ...
        logits = torch.matmul(doc, x.t())  # (batch, num_labels)
        return logits
